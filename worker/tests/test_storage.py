...
Spec: {session_id}/{page_type}/{viewport}/{artifact_type}.{ext}
"""
...
def test_naming_convention_per_spec():
    """
    Naming convention per TECH_SPEC: {session_id}/{page_type}/{viewport}/{artifact_type}.{ext}.
    """
    config = get_config()
    ...
    for artifact_type, ext in ext_map.items():
        path = build_artifact_path(session_id, "homepage", "desktop", artifact_type)
        ...
        assert rel_parts[0] == str(session_id)
    ...
    for artifact_type in artifact_types:
        for page_type in page_types:
            for viewport in viewports:
                path = build_artifact_path(session_id, page_type, viewport, artifact_type)
...
def test_naming_convention_pdp_mobile():
    ...
    session_id = uuid4()
    path = build_artifact_path(session_id, "pdp", "mobile", "screenshot")
...
def test_naming_convention_homepage_desktop():
    ...
    session_id = uuid4()
    path = build_artifact_path(session_id, "homepage", "desktop", "visible_text")
...
def test_artifact_paths_unique_per_session():
    ...
    path1 = build_artifact_path(session_id_1, "homepage", "desktop", "screenshot")
    path2 = build_artifact_path(session_id_2, "homepage", "desktop", "screenshot")
...
def test_artifact_paths_unique_per_page_type():
    ...
    path_home = build_artifact_path(session_id, "homepage", "desktop", "screenshot")
    path_pdp = build_artifact_path(session_id, "pdp", "desktop", "screenshot")
...
def test_artifact_paths_unique_per_viewport():
    ...
    path_desktop = build_artifact_path(session_id, "homepage", "desktop", "screenshot")
    path_mobile = build_artifact_path(session_id, "homepage", "mobile", "screenshot")
...
def test_artifact_paths_unique_per_type():
    ...
    path_screenshot = build_artifact_path(session_id, "homepage", "desktop", "screenshot")
    path_text = build_artifact_path(session_id, "homepage", "desktop", "visible_text")
...
def test_build_artifact_path_deterministic():
    ...
    paths = [
        build_artifact_path(session_id, "homepage", "desktop", "screenshot") for _ in range(10)
    ]
...
def test_artifact_extensions_correct():
    ...
    screenshot = build_artifact_path(session_id, "homepage", "desktop", "screenshot")
    assert screenshot.suffix == ".png"

    text = build_artifact_path(session_id, "homepage", "desktop", "visible_text")
    assert text.suffix == ".txt"

    features = build_artifact_path(session_id, "homepage", "desktop", "features_json")
    assert features.suffix == ".json"

    html = build_artifact_path(session_id, "homepage", "desktop", "html_gz")
...
def test_four_artifacts_per_session_structure():
    ...
    for page_type, viewport in expected_pages:
        screenshot = build_artifact_path(session_id, page_type, viewport, "screenshot")
        text = build_artifact_path(session_id, page_type, viewport, "visible_text")
        features = build_artifact_path(session_id, page_type, viewport, "features_json")
        html = build_artifact_path(session_id, page_type, viewport, "html_gz")
