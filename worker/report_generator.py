...
def _calculate_weighted_category_scores(questions: list[dict]) -> list[dict]:
    ...
    """
    category_data = defaultdict(lambda: {"weighted_score": 0.0, "total_weight": 0.0, "questions": []})
...
def _generate_actionable_findings(questions: list[dict], session_id: UUID, repository) -> list[dict]:
    """
    ...
    for question in failed_questions:
        ...
        if (tier == 1 and severity >= 4) or (tier == 2 and severity == 5):
            impact = "High"
        elif (tier == 1 and severity >= 2) or (tier == 2 and severity >= 3) or (tier == 3 and severity == 5):
            impact = "Medium"
...
def generate_audit_report(session_id: UUID, repository: AuditRepository) -> dict:
    ...
    if page_coverage_score < 4:
        ...
        return {
            ...
            "manual_review_reason": f"Page coverage score {page_coverage_score} is below threshold (4). Insufficient data for reliable report generation.",
            "page_coverage_score": page_coverage_score,
            "questions": [],
            "stage_summaries": [],
            "category_scores": [],
