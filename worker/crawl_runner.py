...
from worker.advance_flow import advance_flow
...
from worker.simple_flow import simple_flow
    ...
    try:
        ...
        while True:
            try:
                ...
                flow_html_content = html_content or await page.content()

                if page_type == "pdp" and flow_html_content:
                    try:
                        config = get_config()
                        checkout_mode = getattr(config, "checkout_processing_mode", "simple")
                        ...
                        logger.info(
                            ...
                            viewport=viewport,
                            domain=domain,
                            checkout_processing_mode=checkout_mode,
                        )
                        repository.create_log(
                            session_id=session_id,
                            level="info",
                            event_type="navigation",
                            message="Checkout flow starting",
                            details={
                                "checkout_processing_mode": checkout_mode,
                                "viewport": viewport,
                                "domain": domain,
                        ...
                        if config.telegram_bot_token and config.telegram_chat_id:
                            try:
                                msg = (
                                    "🧪 Checkout flow starting\n"
                                    f"domain: {domain}\n"
                                    f"viewport: {viewport}\n"
                                    f"session_id: {session_id}\n"
                                    f"mode: {checkout_mode}"
                                )
                        ...
                        if checkout_mode == "simple":
                            repository.create_log(
                                session_id=session_id,
                                level="info",
                                event_type="navigation",
                                message="Simple flow selected",
                                details={"viewport": viewport, "domain": domain},
                            )
                            checkout_result = await simple_flow(
                                page,
                                pdp_url,
                                session_id,
                                viewport,
                            ...
                            repository.create_log(
                                ...
                                level="info",
                                event_type="navigation",
                                message="Advance flow selected",
                                details={"viewport": viewport, "domain": domain},
                            )
                            checkout_result = await advance_flow(
                                page,
                                pdp_url,
                                flow_html_content,
                                page_id,
